...
*/
_specialCases = ["helo1","helo2","helo3","helo4","apache1","apache2","tank1","tank2","tank3","jet"];
if(str player in _specialCases)then{
		...
		player addEventHandler ["getInMan",{
			if((_this select 1) != "cargo")then{
				if((_this select 2) isKindOf "Air")then{
					if(typeOf (_this select 2) == "UK3CB_BAF_Wildcat_AH1_CAS_8A" || typeOf (_this select 2) == "CUP_B_CH47F_GB" || typeOf (_this select 2) == "UK3CB_BAF_Merlin_HC3_32"|| typeOf (_this select 2) == "UK3CB_BAF_Merlin_HC3_CSAR"|| typeOf (_this select 2) == "RHS_CH_47F_light" )then{
	...
	if(str player == "helo3" || str player == "helo4")then{
		player addEventHandler ["getInMan",{
			if((_this select 1) != "cargo")then{
				if((_this select 2) isKindOf "Air")then{
					if(typeOf (_this select 2) == "RHS_UH60M_D" || typeOf (_this select 2) == "RHS_CH_47F_light" || typeOf (_this select 2) == "rhsusf_CH53E_USMC_D" || typeOf (_this select 2) == "RHS_UH1Y_d_GS")then{
	...
	if(str player == "apache1" || str player == "apache2")then{
		...
		if(str player == "jet")then{
		player addEventHandler ["getInMan",{
			if((_this select 1) != "cargo")then{
				if((_this select 2) isKindOf "Air")then{
					if(typeOf (_this select 2) == "CUP_B_C130J_GB" || typeOf (_this select 2) == "CUP_B_C130J_CARGO_GB")then{
			
					}else{
						hint "You are an C-130 Pilot. You may only fly the C-130";
						(_this select 0) action ["eject", (_this select 2)];
						(_this select 0) action ["getout", (_this select 2)];
					};
				};
