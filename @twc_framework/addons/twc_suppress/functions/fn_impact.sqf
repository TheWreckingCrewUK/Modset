...
if (((vehicle player) == player) || (isTurnedOut player) || TWC_Suppress_inExposedVehicle) then {
	if (isWeaponDeployed player) then {
		TWC_Suppress_impactBlur ppEffectAdjust [0.01, 0.1, 0.1, 0.1];
		TWC_Suppress_impactBlur ppEffectCommit 0;
		TWC_Suppress_impactCC ppEffectAdjust [1, 1, 0, [0, 0, 0, 0.125], [1, 1, 1, 1], [1, 1, 1, 0]];
	...
	// Flinch at first shot, if it's been 3 minutes since last being shot at
	if ((time - TWC_Suppress_lastShotTime) >= 180) then {
