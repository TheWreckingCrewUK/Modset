...
private _fnc_newMag = {
	...
	_events pushBack [_time, false, +_arrayOfAmmoCounts, _ammoTrans];
};

private _currentMagIter = 0;
private _ammoTrans = 0;
private _index = 0;
private _ammoAvailable = 0;
private _time = 0;
private _events = [];

while {_currentMagIter < (_numberOf - 1)} do {
	...
	if (_ammoAvailable == 0) then {
		...
		for "_swapProgress" from 0 to (_ammoAvailable - 1) do {
			...
			_ammoTrans = _ammoTrans + 1;
			_arrayOfAmmoCounts set [_index, (_arrayOfAmmoCounts select _index) - 1];
			_events pushBack [_time, true, +_arrayOfAmmoCounts, _ammoTrans];
		};
	};
};
