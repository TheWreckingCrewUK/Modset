...
private _lastTimeUpdated = _unit getVariable ["ace_medical_vitals_lastTimeUpdated", 0];
...
_unit setVariable ["ace_medical_vitals_lastTimeUpdated", CBA_missionTime];
private _lastTimeValuesSynced = _unit getVariable ["ace_medical_vitals_lastMomentValuesSynced", 0];
...
if (_tourniquetPain > 0) then {
	[_unit, _tourniquetPain] call ace_medical_status_fnc_adjustPainLevel;
};
...
private _heartRate = [_unit, _hrTargetAdjustment, _deltaT, _syncValues] call ace_medical_vitals_fnc_updateHeartRate;
[_unit, _painSupressAdjustment, _deltaT, _syncValues] call ace_medical_vitals_fnc_updatePainSuppress;
[_unit, _peripheralResistanceAdjustment, _deltaT, _syncValues] call ace_medical_vitals_fnc_updatePeripheralResistance;
...
switch (true) do {
	case (_bloodVolume < BLOOD_VOLUME_FATAL): {
		["ace_medical_Bleedout", _unit] call CBA_fnc_localEvent;
	};
	...
	case (_heartRate < 30): {  // With a heart rate below 30 but bigger than 20 there is a chance to enter the cardiac arrest state
		private _nextCheck = _unit getVariable ["ace_medical_vitals_nextCheckCriticalHeartRate", CBA_missionTime];
		...
		if (CBA_missionTime >= _nextCheck) then {
			...
			_unit setVariable ["ace_medical_vitals_nextCheckCriticalHeartRate", CBA_missionTime + 5];
		};
