...
if (isNil "InsP_cacheGroup") then {
...

	{
		while {(_x distance (getMarkerPos "cacheSpawn")) < 50} do {
			...
			switch (str _forEachIndex) do {
				case "0": {_cacheMarker = cacheBoxAPos};
				case "1": {_cacheMarker = cacheBoxBPos};
				case "2": {_cacheMarker = cacheBoxCPos};
			};
	
			_houseList = (getPos _cacheMarker) nearObjects ["House",1000];
			sleep .25;
			_c = 0;
	...
	
	{
		...
		_trg setTriggerStatements ["(((objectParent (thisList call bis_fnc_selectRandom)) isKindOf 'air') || (getPosATL (thisList call bis_fnc_selectRandom)) select 2 < 25)","[(getPos thisTrigger),4,150,2,[300,400],thisList] call twc_townSetup",""];
	}forEach InsP_cacheGroup;
};