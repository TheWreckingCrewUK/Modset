...
_trg2 setTriggerStatements ["((EAST countSide thisList) < 4 && {_x isKindof 'LandVehicle' || _x isKindof 'air' && side _x == WEST} count thisList == 0) OR (West countSide thisList) == 0", format["['%1', thisList] call twc_siteDeciding", _marker], ""];