//US Army
SpawnM2A3Action = ["SpawnM2A3","M2A3","",{_oldVeh = nearestObjects [armourSpawnPad, ["Car","Tank","air"],10]; {deleteVehicle _x}forEach _oldVeh; _veh = "CUP_B_M2A3Bradley_USA_D" createVehicle (position armourSpawnPad);_dir = getDir armourSpawnPad; _dir = _dir + 180; _veh setDir _dir;armourSpawned = 1;},{TRUE}] call ace_interact_menu_fnc_createAction;
...
SpawnM113A3action = ["SpawnM113A3","M113A3","",{_oldVeh = nearestObjects [jetSpawnPad, ["Car","Tank"],10]; {deleteVehicle _x}forEach _oldVeh; _veh = "CUP_B_M113_USA" createVehicle (position jetSpawnPad);_dir = getDir jetSpawnPad; _dir = _dir + 180; _veh setDir _dir; [_veh, 180, 1200, 0, FALSE] execVM "server\vehicles\vehicleRespawn.sqf"},{TRUE}] call ace_interact_menu_fnc_createAction;
["Land_InfoStand_V1_F", 0, ["ACE_MainActions"], SpawnM113A3Action] call ace_interact_menu_fnc_addActionToClass;

SpawnHMMVM2Action = ["SpawnHMMVM2","HMMV M2","",{_oldVeh = nearestObjects [jetSpawnPad, ["Car","Tank"],10]; {deleteVehicle _x}forEach _oldVeh; _veh = "CUP_B_HMMWV_M2_USA" createVehicle (position jetSpawnPad);_dir = getDir jetSpawnPad; _dir = _dir + 180; _veh setDir _dir; [_veh, 180, 1200, 0, FALSE] execVM "server\vehicles\vehicleRespawn.sqf"},{TRUE}] call ace_interact_menu_fnc_createAction;
["Land_InfoStand_V1_F", 0, ["ACE_MainActions"], SpawnHMMVM2Action] call ace_interact_menu_fnc_addActionToClass;

SpawnHMMVMK19Action = ["SpawnHMMVMK19","HMMV MK19","",{_oldVeh = nearestObjects [jetSpawnPad, ["Car","Tank"],10]; {deleteVehicle _x}forEach _oldVeh; _veh = "CUP_B_HMMWV_MK19_USA" createVehicle (position jetSpawnPad);_dir = getDir jetSpawnPad; _dir = _dir + 180; _veh setDir _dir; [_veh, 180, 1200, 0, FALSE] execVM "server\vehicles\vehicleRespawn.sqf"},{TRUE}] call ace_interact_menu_fnc_createAction;
