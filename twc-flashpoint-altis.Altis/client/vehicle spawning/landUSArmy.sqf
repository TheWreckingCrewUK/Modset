//US Army
SpawnStrykerAction = ["SpawnStryker","Stryker M2","",{_veh = "CUP_B_M1126_ICV_M2_Desert_Slat" createVehicle (position jetSpawnPad);_dir = getDir jetSpawnPad; _dir + 180; _veh setDir _dir; [_veh, 180, 1200, 0, FALSE] execVM "server\vehicles\vehicleRespawn.sqf"},{TRUE}] call ace_interact_menu_fnc_createAction;
["Land_InfoStand_V1_F", 0, ["ACE_MainActions"], SpawnStrykerAction] call ace_interact_menu_fnc_addActionToClass;

SpawnM113A3action = ["SpawnM113A3","M113A3","",{_veh = "CUP_B_M113_USA" createVehicle (position jetSpawnPad);_dir = getDir jetSpawnPad; _dir + 180; _veh setDir _dir; [_veh, 180, 1200, 0, FALSE] execVM "server\vehicles\vehicleRespawn.sqf"},{TRUE}] call ace_interact_menu_fnc_createAction;
...
SpawnHMMVTOWAction = ["SpawnHMMVTOW","HMMV TOW","",{_veh = "CUP_B_HMMWV_TOW_USA" createVehicle (position jetSpawnPad);_dir = getDir jetSpawnPad; _dir + 180; _veh setDir _dir; [_veh, 180, 1200, 0, FALSE] execVM "server\vehicles\vehicleRespawn.sqf"},{TRUE}] call ace_interact_menu_fnc_createAction;
