...
class Extended_HitPart_EventHandlers {
	class man {
		class twc_vitalhit {
		HitPart = "(_this select 0) params ['_target', '_shooter', '_projectile', '_position', '_velocity', '_selection', '_ammo', '_vector', '_radius', '_surfaceType', '_isDirect'];if (isplayer _target) exitwith {_this call twc_fnc_playerheadshot};if (!alive _target) exitwith {};if (!_isdirect) exitwith {}; _value = (_ammo select 0); _armour = (getNumber (configFile >> 'CfgWeapons' >> vest _target >> 'iteminfo' >> 'HitpointsProtectionInfo' >> 'Chest' >> 'armor')); if (_value > 20) exitwith {[_target, {if (isDamageAllowed _this) then {_this setdamage 1;};}] remoteExec ['call', _target];};if (('head' in _selection) || (('neck' in _selection) && ((random 1) > 0.5))) exitwith {[_target, {if (isDamageAllowed _this) then {_this setdamage 1;};}] remoteExec ['call', _target];};if (_value > 6.5) then {if (('spine3' in _selection) && ((_armour == 0) || ((lifeState _target) != 'HEALTHY') || (_value > (8 + ((_target distance _shooter) * 0.008))))) exitwith {[_target, {if (isDamageAllowed _this) then {_this setdamage 1;};}] remoteExec ['call', _target];};} else { if ((((('spine1' in _selection) || ('spine2' in _selection) || ('spine3' in _selection)) && (_armour == 0)) || ('neck' in _selection)) && (((lifeState _target) != 'HEALTHY'))) exitwith {[_target, {if (isDamageAllowed _this) then {_this setdamage 1;};}] remoteExec ['call', _target];};};";
		};
	};
};
