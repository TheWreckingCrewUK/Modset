...
if (player in Trainers) then {
	execVM "weaponLists\playerLoadouts\modernTrainer.sqf";
	_markerList = ['markerList','Area Markers List','', {hint format ["%1\n%2\n%3\n%4\n%5\n%6\n%7",'rifeRange','atArea','cqbArea','sniperRange','vehicleTraining','hqArea','battleship'];},{true}] call ace_interact_menu_fnc_createAction;
	[player, 1, ["ACE_SelfActions"], _markerList] call ace_interact_menu_fnc_addActionToObject;

}else{
	execVM "weaponLists\playerLoadouts\modern.sqf";
};