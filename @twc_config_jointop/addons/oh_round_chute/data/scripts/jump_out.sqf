...
if (_veh isKindOf "CAF_CH146_GRIFFON_J") then {
			...
			if (((getPosASL _veh select 2) <50) or ((getPos _veh select 2) < 50)) then {
				...
				_caller attachTo [_veh,[0,0,0],"pos Cargo L"]; 
			...
			moveOut _caller;
			_caller setdir _newDir;
			_caller attachTo [_veh,[-4,-2,-2],"pos Cargo L"]; 
...
if (_veh isKindOf "CH_147F") then {
			//HINT "this is the right script";
			//_pos = getPosASL _veh;
			//_exitpos = [(_pos select 0) +25, _pos select 1, (_pos select 2) -2];
			//_exitpos = [(_pos select 0)+5, (_pos select 1) +25, _pos select 2];
			if (((getPosASL _veh select 2) <50) or ((getPos _veh select 2) < 50)) then {
			...
			//If Jumper has a ruck, hang it below him
			if (!isNull (unitBackpack _caller)) then {
						private ["_pack","_class","_magazines","_weapons","_items"];
						_pack	   = unitBackpack _caller;
						_class	   = typeOf _pack;
			...
			//Auto Open the Chute
			//_caller setPosASL [getposASL _caller select 0, getposASL _caller select 1, getposASL _veh select 2];
			sleep 1;
			_caller action ["OpenParachute",_caller];
			_caller say3D "ChuteOpen";
			//Get more info
