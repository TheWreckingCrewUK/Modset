class CfgSounds {
	sounds[] = {};
	
	class twc_banzai_1 {
		name = "banzai_1";
		sound[] = {"twc_flavour\sounds\ww2\banzai_1.ogg", 1, 1};
		titles[] = {};
	};
	
	class twc_banzai_2: twc_banzai_1 {
		name = "banzai_2";
		sound[] = {"twc_flavour\sounds\ww2\banzai_2.ogg", 1, 1};
	};

	class twc_banzai_3: twc_banzai_1 {
		name = "banzai_2";
		sound[] = {"twc_flavour\sounds\ww2\banzai_2.ogg", 1, 1};
	};
	
	class twc_banzai_4: twc_banzai_1 {
		name = "banzai_2";
		sound[] = {"twc_flavour\sounds\ww2\banzai_2.ogg", 1, 1};
	};
	
	class twc_banzai_5: twc_banzai_1 {
		name = "banzai_2";
		sound[] = {"twc_flavour\sounds\ww2\banzai_2.ogg", 1, 1};
	};
	
	class twc_radio_chatter {
		name = "radio_chatter";
		sound[] = {"twc_flavour\sounds\radio\radio_chatter.ogg", 1, 1};
		titles[] = {};
	};
	
	class twc_bugle_call {
		name = "bugle_call";
		sound[] = {"twc_flavour\sounds\other\bugle_call.ogg", 1, 1};
		titles[] = {};
	};
	
	class twc_arab_crowd {
		name = "arab_crowd";
		sound[] = {"twc_flavour\sounds\crowds\arab_crowd.ogg", 1, 1};
		titles[] = {};
	};
	
	class twc_sound_airraid_warning {
		name = "airraid_warning";
		sound[] = {"twc_flavour\sounds\alarms\air_raid_warning.ogg", 1, 1};
		titles[] = {};
	};
	
	class twc_sound_airraid_allclear {
		name = "airraid_allclear";
		sound[] = {"twc_flavour\sounds\alarms\air_raid_allclear.ogg", 1, 1};
		titles[] = {};
	};
};