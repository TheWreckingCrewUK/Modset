/* == TASKMASTER ===============================================================================
  ...
  Forum: http://forums.bistudio.com/showthread.php?160974-SHK_fnc
  relabled the functions just for ease of use by FakeMatty

  == Creating a briefing =======================================================================

  Tasks and notes are given in the init.sqf.

  Syntax: [[[Task1Data],[Task2Data]],[[Note1Data],[Note2Data]]] execvm "TWC_fnc.sqf";
  Example:
    [[
  ...
  Example: ["Task1","succeeded"] call TWC_fnc_UpdateTask;

  It's possible to set state of one task and set another as assigned using an optional 3rd parameter.
  Example: ["Task1","succeeded","Task2"] call TWC_fnc_UpdateTask;
  ...
  Example: ["Task1","succeeded",["Task2","Title","Desc"]] call TWC_fnc_UpdateTask;

  This will make task Task1 as succeeded and create a new task Task2. Same set of parameters is used for the
  creation as in init.sqf or TWC_fnc_CreateTask.
  ...
  Example: ["Task2","Extraction","Get to teh choppa!"] call TWC_fnc_CreateTask;

  == Checking task status ======================================================================

    TWC_fnc_areCompleted
      ...
      Example: ["Task1","Task2"] call TWC_fnc_isCompleted

    TWC_fnc_isCompleted
      This function can be used to check if a task is completed. Task is considered completed with states
      succeeded, failed and canceled. Function returns a boolean (true/false) value.

      Example: "Task1" call TWC_fnc_isCompleted
      Multiple tasks: ["Task1","Task2"] call TWC_fnc_isCompleted

    TWC_fnc_getAssigned
      Returns list of tasks which have "assigned" as their state.

      Example: call TWC_fnc_getAssigned
      Example result: ["Task1","Task4"]

    TWC_fnc_getState
      Returns the task state (succeeded/failed/canceled/assigned/created).

      Example: "Task1" call TWC_fnc_getState

    TWC_fnc_hasState
      Checks if a task's state matches the given state. Function returns a boolean value.

      Example: ["Task1","succeeded"] call TWC_fnc_hasState

    TWC_fnc_hasTask
      Checks if a task with the given name has been created. Returns boolean.

      Example: "Task1" call TWC_fnc_hasTask

    TWC_fnc_setCurrentLocal
      Sets the given task as the current task.

      Example: "Task1" call TWC_fnc_setCurrentLocal;
      Multiplayer Example: ["Task1","TWC_fnc_setCurrentLocal",true,true] call BIS_fnc_MP

    TWC_fnc_CreateTaskNote (client only)
      ...
      Example: Example: ["Enemy forces","Oh noes, there will be enemy soldiers in the area of operation."] call TWC_fnc_CreateTaskNote
  ...
  == Version history ======================================================================
    ...
    0.38  Added: TWC_fnc_isCompleted now takes multiple tasks are parameter. Also, added _areCompleted as a wrapper function.
    0.37  Changed: Task creation order was reversed in Arma 2. It was changed in Arma 3. Changed the order in script to be correct with A3. Automatic current task assignment was removed, use the TWC_fnc_setCurrentLocal.
    0.36  Added: TWC_fnc_setCurrentLocal can now be used to set a task as current task. Only works locally, so use with BIS_fnc_MP.
    ...
    0.13  Added: TWC_fnc_initDone variable will be set to true once tasks are processed and functions loaded.
...
/* == COMMON =================================================================================== */
  ...
  TWC_fnc_CreateTask = {
    ...
    */
    if isserver then {
	  ...
	  params ["_name","_short","_long",["_Primary",True],["_cond",true],["_marker",[]],["_state","created"],["_dest", 0]];
	  if _Primary then {
	  _short = "[P] " + _short;
	  }else{
	  _short = "[S] " + _short;
      ...
      */
      if (!isdedicated && TWC_fnc_initDone) then {
        TWC_fnc_Tasks spawn TWC_fnc_handleEvent;
      };
    };
  };
  TWC_fnc_CreateTaskNote = {
  ...
  TWC_fnc_CreateTaskTask = {
    ...
    */
    ...
    if DEBUG then { diag_log format ["TWC_fnc> addTask: %1, %2, %3, %4",_name,_marker,_state,_dest]};
    {
      if ( [_x,(_this select 3)] call TWC_fnc_checkCond ) then {
        ...
        if (_x == player) then {
          if (TWC_fnc_showHints) then { [_handle,_state] call TWC_fnc_showHint };
    ...
    TWC_fnc_TasksLocal set [count TWC_fnc_TasksLocal,[_name,_state,_handles]];
  };
  TWC_fnc_areCompleted = {
    ...
    */
    private ["_b"];
    _b = false;
    _b = _this call TWC_fnc_isCompleted;
    _b;
  };
  TWC_fnc_assign = {
    ...
    */
    if isserver then {
      private "_task";
      for "_i" from 0 to (count TWC_fnc_Tasks - 1) do {
        if (_this == ((TWC_fnc_Tasks select _i) select 0)) then {
          _task =+ TWC_fnc_Tasks select _i;
          _task set [5,"assigned"];
          TWC_fnc_Tasks set [_i,_task];
        };
      };
      publicvariable "TWC_fnc_Tasks";
      /*
  ...
  TWC_fnc_checkCond = {
    ...
    */
    ...
    if (!isNil "_cond") then {
      if DEBUG then { diag_log format ["TWC_fnc> typename condition: %1",typename _cond]};
      switch (typename _cond) do {
        ...
        case (typename ""): {
          if (_cond call TWC_fnc_isFaction) then {
            (faction _unit == _cond)
          } else {
  ...
  TWC_fnc_handleEvent = {
    /*   Takes the pubvar'd task array and determines if a task needs to be added or updated.
     In: array     TWC_fnc_Tasks from pubvar eventhandler
    Out:
    */
    ...
    {
      _name = _x select 0;
      if (_name call TWC_fnc_hasTaskLocal) then {
        if ([_name,(_x select 5)] call TWC_fnc_hasStateChanged) then {
          if DEBUG then { diag_log format ["TWC_fnc> handleEvent calling updateTask: %1",_name]};
          _x call TWC_fnc_UpdateTaskateTask;
        };
      } else {
        if DEBUG then { diag_log format ["TWC_fnc> handleEvent calling addTask: %1",_name]};
        _x call TWC_fnc_CreateTaskTask;
      };
    } foreach _this;
  };
  TWC_fnc_hasState = {
    ...
    */
    ...
    _b = false;
    {
      if ((_this select  0) == (_x select 0)) then {
        if (((_this select 0) call TWC_fnc_getState) == (_this select 1)) then {
          _b = true;
        };
      };
    } foreach TWC_fnc_Tasks;
        ...
        private "_handle";
        {
          _handle = _x;
          {
            if (_handle in (simpletasks _x)) then {
              ...
              if (_x == player) then {
                if (TWC_fnc_showHints) then { [_handle,"Assigned"] call TWC_fnc_showHint };
              };
            };
          } foreach (if ismultiplayer then {playableunits} else {switchableunits});
        } foreach (_x select 2);
    ...
    if isserver then {
      ...
      if (count _this > 2) then {
        switch (typename (_this select 2)) do {
          case (typename ""): { (_this select 2) call TWC_fnc_assign };
          case (typename []): { (_this select 2) spawn { sleep 1; _this call TWC_fnc_CreateTask} };
        };
      };
      publicvariable "TWC_fnc_Tasks";
      /*
  ...
  TWC_fnc_UpdateTaskateTask = {
    ...
    */
    ...
    for "_i" from 0 to (count TWC_fnc_TasksLocal - 1) do {
      _task =+ TWC_fnc_TasksLocal select _i;
      _name = _task select 0;
      if (_name == (_this select 0)) then {
        _marker = _this select 4;
        _state = _this select 5;
        _task set [1,_state];
        TWC_fnc_TasksLocal set [_i,_task];
        {
          _handle = _x;
          {
            if (_handle in (simpletasks _x)) then {
              ...
              if (_x == player) then {
                ...
                if (count _marker > 0) then {
                  if (_state in ["succeeded","failed","canceled"]) then {
                    if DEBUG then { diag_log format ["TWC_fnc> updateTask deleting marker: %1, state: %2",_marker,_state]};
                    if (typename (_marker select 0) == typename "") then {
                      _marker = [_marker];
                    };
                    {
...
/* == SERVER =================================================================================== */
if isserver then {
  TWC_fnc_Tasks = []; // Array member: ["Name","Title","Desc","Marker","State"]
  /*
  ...
  */
  if (!isnil "_this") then {
    if (count _this > 0) then {
      ...
      {
        _x call TWC_fnc_CreateTask;
      } foreach _tasks;
    };
  };
  publicvariable "TWC_fnc_Tasks";
  if DEBUG then {
    diag_log "-- TWC_fnc_Tasks --";
    diag_log TWC_fnc_Tasks;
  };
...
/* == CLIENT =================================================================================== */
if !isdedicated then {
  TWC_fnc_showHints = false;
  TWC_fnc_TasksLocal = []; // Array member: ["TaskName","TaskState",TaskHandles]
  /*
  ...
  if (!isnil "_this") then {
    if (count _this > 1) then {
      ...
      for [{_i=(count _notes - 1)},{_i>-1},{_i=_i-1}] do {
        (_notes select _i) call TWC_fnc_CreateTaskNote;
      };
    };
  };
  /*
  ...
  */
  [] spawn {
    ...
    waituntil {!isnil "TWC_fnc_Tasks"};
    if DEBUG then {diag_log format ["TWC_fnc> Tasks received first time: %1",TWC_fnc_Tasks]};
    private "_sh";
    _sh = TWC_fnc_Tasks spawn TWC_fnc_handleEvent;
    waituntil {scriptdone _sh};

    TWC_fnc_showHints = true;
    TWC_fnc_initDone = true;

    "TWC_fnc_Tasks" addpublicvariableeventhandler {
      (_this select 1) spawn TWC_fnc_handleEvent;
    };
  };
};