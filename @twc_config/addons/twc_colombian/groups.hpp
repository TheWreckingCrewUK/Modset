class TWC_Groups_Colombia
{
	name = "Infantry";
	class Squad
	{
		name = "Squad";
		faction = "TWC_Colombia";
		side = 1;
		class Unit0
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_SquadLeader";
				rank = "SERGEANT";
				position[] = {0,0,0};
		};
		class Unit1
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_TeamLeader";
				rank = "CORPORAL";
				position[] = {2,0,0};
		};
		class Unit2
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_Rifleman";
				rank = "PRIVATE";
				position[] = {4,0,0};
		};
		class Unit3
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_RiflemanAT";
				rank = "PRIVATE";
				position[] = {6,0,0};
		};
		class Unit4
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_Grenadier";
				rank = "PRIVATE";
				position[] = {8,0,0};
		};
		class Unit5
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_MG";
				rank = "PRIVATE";
				position[] = {10,0,0};
		};
		class Unit6
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_TeamLeader";
				rank = "CORPORAL";
				position[] = {12,0,0};
		};
		class Unit7
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_Rifleman";
				rank = "PRIVATE";
				position[] = {14,0,0};
		};
		class Unit8
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_RiflemanAT";
				rank = "PRIVATE";
				position[] = {16,0,0};
		};
		class Unit9
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_Grenadier";
				rank = "PRIVATE";
				position[] = {18,0,0};
		};
		class Unit10
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_MG";
				rank = "PRIVATE";
				position[] = {20,0,0};
		};
	};
	
	class Platoon_Command
	{
		name = "Platoon Command";
		faction = "TWC_Colombia";
		side = 1;
		class Unit0
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_PlatoonLeader";
				rank = "LIEUTENANT";
				position[] = {0,0,0};
		};
		class Unit1
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_PlatoonSergeant";
				rank = "SERGEANT";
				position[] = {2,0,0};
		};
		class Unit2
		{
				side = 1;
				vehicle = "TWC_Infantry_Colombia_PlatoonMedic";
				rank = "PRIVATE";
				position[] = {4,0,0};
		};
	};
};